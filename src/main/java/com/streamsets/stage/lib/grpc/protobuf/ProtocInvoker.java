...
public class ProtocInvoker {
  ...
  public static ProtocInvoker forConfig(GrpcConfigBean conf) {
    Preconditions.checkArgument(conf.protoDiscoveryRoot().isPresent(),
        "A proto discovery root is required for proto analysis");
    Path discoveryRootPath = conf.protoDiscoveryRoot().orElse(Paths.get("").toAbsolutePath());
    Preconditions.checkArgument(discoveryRootPath.toFile().exists(),
        "Invalid proto discovery root path: " + discoveryRootPath);

    ImmutableList.Builder<Path> includePaths = ImmutableList.builder();
    for (Path path : conf.additionalProtocIncludes().asList()) {
      Preconditions.checkArgument(path.toFile().exists(), "Invalid proto include path: " + path);
  ...
  private void invokeBinary(ImmutableList<String> protocArgs) throws ProtocInvocationException {
    ...
    try( ByteArrayOutputStream protocStdout = new ByteArrayOutputStream()) {
